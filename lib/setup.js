// First-run setup wizard (/voice): makes sure the selected STT backend is ready,
// picks the LLM that cleans up dictation, and picks the input mode. Installs and
...
import { STT_BACKENDS, STT_MODELS, resolveSttConfig } from "./stt-engines.js";
import {
...
import { BACK_KEY, addBackRow, createMenu, shortLabel } from "./menu.js";
import { UI_LANGUAGE_OPTIONS, normalizeUiLocale, translate } from "./i18n.js";
import {
  listCleanupModels,
  probeCleanupModels,
  rankProbeResults,
...
// Keep the guided path actionable: each first choice has a local installer
// rather than requiring the user to bring an external service.
const GUIDED_RECOMMENDED_MODELS = {
  en: "nemo:parakeet-tdt",
  es: "nemo:parakeet-tdt",
  zh: "remote:fun-asr-nano-2512",
  ja: "nemo:nemotron-3.5",
  ko: "nemo:nemotron-3.5",
};
...
export function jobLine(job, now = Date.now(), labelWidth = 0) {
  ...
  if (job.state === "pending") return `${label}  waiting${detail}`;
  if (job.percent == null && job.noSpinner) {
    return `${label}  ${job.detail || "setting up"} · ${secs}s`;
  }
...
/** Keep guided setup focused on dictation; cleanup remains configurable in /voice. */
export function configureGuidedCleanup(kv, resolved) {
  const mode = kv.get("cleanup.mode");
  if (mode === "skip" || (mode === "opencode" && kv.get("cleanup.model"))) return;
  if (resolved.available && resolved.model) {
    kv.set("cleanup.mode", "opencode");
    kv.set("cleanup.model", resolved.model);
    return;
  }
...
export function createVoiceSetup(ctx) {
  ...
  let guidedLocale = normalizeUiLocale(kv.get("voice.locale", kv.get("stt.language", "en")));
  let guidedLanguage = kv.get("stt.language", guidedLocale) || guidedLocale;
  let guidedMode = getVoiceMode?.() || kv.get("voice.mode", "hold");

  function guidedText(key, variables) {
    return translate(guidedLocale, key, variables);
  }

  function addGuidedBackRow(options, back) {
    if (!back) return options;
    options.push({
      title: `← ${guidedText("back")}`,
      footer: BACK_KEY,
      value: "back",
      onSelect: back,
    });
  ...
  function guidedDialog({
    ...
    menu({ title, options, back, current, filter, placeholder, size });
  }

  function stepGuidedLanguage({ back } = {}) {
    const current = guidedLanguage;
    const options = [...UI_LANGUAGE_OPTIONS]
      .sort((a, b) => {
      ...
      .map((language) => ({
        ...
        description: language.nativeLabel === language.label ? undefined : language.label,
        footer: language.id === current ? guidedText("selected") : undefined,
        value: language.id,
        onSelect() {
          guidedLocale = normalizeUiLocale(language.id);
          guidedLanguage = language.id;
          kv.set("voice.locale", guidedLocale);
          kv.set("stt.language", language.id);
          const showLanguage = () => stepGuidedLanguage({ back });
          const showKeyCapture = () =>
            startGuidedKeyCapture({
              back: showLanguage,
              next: () => stepGuidedMode({ back: showKeyCapture }),
            });
          showKeyCapture();
        },
      }));
    addGuidedBackRow(options, back);
    guidedDialog({
      title: guidedText("language"),
      options,
      back,
      current,
    });
  ...
  function saveGuidedKey(value, next, beforeClose) {
    if (!setTalkKey?.(value)) return false;
    beforeClose?.();
    api.ui.dialog.clear();
    queueMicrotask(next);
    return true;
  ...
  function captureGuidedKey({ next, back } = {}) {
    if (
      ...
      if (typeof api.ui.DialogPrompt !== "function") {
        toast(guidedText("keyHint"), "warning");
        return;
      }
      let accepted = false;
      api.ui.dialog.replace(
        () =>
          api.ui.DialogPrompt({
            title: guidedText("key"),
            value: getTalkKey(),
            placeholder: "ctrl+r, f2, or <leader>v",
            onConfirm(value) {
              saveGuidedKey(value, next, () => {
                accepted = true;
              });
            },
            onCancel() {
        ...
        () => {
          if (!accepted) queueMicrotask(() => back?.());
        },
      );
        ...
        () =>
          api.ui.DialogAlert({
            ...
            onConfirm() {
              ...
              accepted = saveGuidedKey(pendingKey, next, close);
              if (accepted) pendingKey = null;
            },
          }),
        () => {
          if (replacing) return;
    ...
    disposeCapture = api.keymap.intercept(
      "key",
      ({ event, consume }) => {
        ...
        if (controlKey === "escape" || controlKey === "esc") {
          close();
          api.ui.dialog.clear();
          return;
        }
        if (controlKey === "enter" || controlKey === "return") {
  ...
  function startGuidedKeyCapture({ back, next } = {}) {
    captureGuidedKey({
      back,
      next,
    });
  ...
  function stepGuidedMode({ back } = {}) {
    const choose = (mode) => {
      guidedMode = mode;
      setVoiceMode(mode, { silent: true });
      stepGuidedModels({ back: () => stepGuidedMode({ back }) });
    };
    const options = [
      {
        title: guidedText("hold"),
        description: guidedText("holdHint"),
        value: "hold",
        onSelect() {
          choose("hold");
        },
      ...
      {
        title: guidedText("tap"),
        description: guidedText("tapHint"),
        value: "tap",
        onSelect() {
          choose("tap");
        },
  ...
  function applyGuidedModel(id, option) {
    kv.set("stt.model", id);
    kv.set("stt.backend", option.backend);
    kv.set("stt.endpoint", "");
    kv.set("stt.apiKeyEnv", "");
    if (option.backend === STT_BACKENDS.NEMO) {
      kv.set("stt.nemo.model", option.model);
      return;
    }
    kv.set("stt.remote.model", option.model);
  }

  function stepGuidedModels({ back } = {}) {
    const recommended = GUIDED_RECOMMENDED_MODELS[guidedLanguage] ?? "nemo:parakeet-tdt";
    const options = Object.entries(STT_MODELS)
      .filter(([, option]) => option.languages == null || option.languages.includes(guidedLanguage))
      .sort(([left], [right]) => Number(right === recommended) - Number(left === recommended))
      .map(([id, option]) => ({
        title: option.short,
        description: option.detail,
        footer: id === recommended ? guidedText("recommended") : undefined,
        value: id,
        onSelect() {
          applyGuidedModel(id, option);
          runEngineSteps({
            back,
            next: () => finishGuidedSetup(),
          });
        },
      }));
    addGuidedBackRow(options, back);
    guidedDialog({
      title: guidedText("model"),
      options,
      back,
      current: resolveSttConfig(kv, opts).id,
      size: "large",
    });
  }

  function finishGuidedSetup() {
    setVoiceMode(guidedMode, { silent: true });
    kv.set("voice.locale", guidedLocale);
    kv.set("stt.language", guidedLanguage);
    kv.set("voice.setupDone", true);
    const instruction =
      guidedMode === "tap"
        ? guidedText("readyTap", { key: getTalkKey() })
        : guidedText("readyHold", { key: getTalkKey() });
    guidedRun = false;
    if (typeof api.ui.DialogAlert !== "function") {
      api.ui.dialog.clear();
      toast(`${guidedText("ready")} · ${instruction}`, "success");
      return;
    }
    api.ui.dialog.replace(() =>
      api.ui.DialogAlert({
        title: guidedText("ready"),
        message: `${guidedText("readyHint")}\n\n${instruction}`,
        onConfirm() {
          api.ui.dialog.clear();
        },
      }),
  ...
  function visiblePanelJobs() {
    return panelJobs().flatMap((job) => job.panelJobs || [job]);
  }
  ...
  function fileBytes(file) {
    let bytes = 0;
    for (const candidate of [file, `${file}.partial`]) {
      try {
        bytes = Math.max(bytes, fs.statSync(candidate).size);
      } catch {}
    }
    return bytes;
  }

  function stageBytes(files) {
    return files.reduce((total, file) => total + fileBytes(file), 0);
  }

  function runtimeStageValue(job) {
    try {
      return fs.readFileSync(job.stageFile, "utf8").trim();
    } catch {
      return "";
    }
  }

  function refreshRuntimeJob(job) {
    if (!job.panelJobs) return;
    if (job.modelCached) {
      for (const stageJob of job.panelJobs) {
        if (job.state === "running") stageJob.state = "running";
      }
      return;
    }
    const stage = runtimeStageValue(job);
    for (const stageJob of job.panelJobs) {
      const complete = stage === "ready" || (stage === "weights" && stageJob.id === "endpoint");
      const running = stage === stageJob.id;
      if (complete) {
        stageJob.state = "done";
        stageJob.percent = stageJob.totalBytes ? 100 : null;
      } else if (running && job.state === "running") {
        stageJob.state = "running";
        if (stageJob.cacheModel && stageJob.totalBytes) {
          const modelState = managedSttModelState(job.backend, job.model);
          stageJob.percent = Math.min(99, (modelState.bytes / stageJob.totalBytes) * 100);
        } else if (stageJob.totalBytes) {
          stageJob.percent = Math.min(99, (stageBytes(stageJob.files) / stageJob.totalBytes) * 100);
        }
      } else if (job.state === "running") {
        stageJob.state = "pending";
        stageJob.percent = null;
      }
    }
  }

  function finishRuntimeJob(job, ok, reason) {
    if (!job.panelJobs) return;
    const active =
      job.panelJobs.find((stageJob) => stageJob.state === "running") ||
      job.panelJobs.find((stageJob) => stageJob.state !== "done");
    for (const stageJob of job.panelJobs) {
      if (ok) {
        stageJob.state = "done";
        stageJob.percent = stageJob.totalBytes ? 100 : null;
        stageJob.finishedAt = job.finishedAt;
      } else if (stageJob === active) {
        stageJob.state = "failed";
        stageJob.error = reason;
        stageJob.finishedAt = job.finishedAt;
      }
    }
  }
  ...
  function paintPanel() {
    for (const job of jobs.values()) refreshJob(job);
    const list = visiblePanelJobs();
  ...
  function cancelJob(job) {
    ...
    const signal = job.kind === "runtime" ? "SIGTERM" : "SIGKILL";
    try {
      process.kill(-job.proc.pid, signal);
    } catch {
      try {
  ...
  function runJob({
    ...
    if (existing?.state === "running") {
      if (onDone) existing.onDoneCallbacks.push(onDone);
    ...
    const job = {
      ...
      error: null,
      modelCached,
      onDoneCallbacks: onDone ? [onDone] : [],
    };
    if (kind === "runtime") {
      const progress = managedSttRuntimeProgress(backend, undefined, model);
      if (progress) {
        job.stageFile = managedSttRuntimePaths(backend).stage;
        job.panelJobs = Object.entries(progress).map(([id, stage]) => ({
          key: `${key}:${id}`,
          id,
          label: `${label} ${id}`,
          state: id === "endpoint" ? "running" : "pending",
          startedAt,
          finishedAt: null,
          percent: stage.totalBytes ? 0 : null,
          totalBytes: stage.totalBytes,
          files: stage.files,
          detail: stage.detail ?? null,
          noSpinner: stage.noSpinner === true,
          cacheModel: stage.cacheModel === true,
          tick: 0,
          error: null,
        }));
      }
    ...
    const finish = (ok, reason) => {
      ...
      job.error = ok ? null : reason;
      finishRuntimeJob(job, ok, reason);
      invalidateBinaryCache();
      logger?.log("STT", `Setup job ${job.state}: ${label}${ok ? "" : ` (${reason})`}`, "debug");
      paintPanel();
      for (const callback of job.onDoneCallbacks) callback(ok);
    };
    ...
    proc.on("exit", (code) => {
      ...
      finish(false, last?.slice(0, 160) || `exit ${code}`);
    });
  ...
  function downloadModel(model, { onDone } = {}) {
    ...
    if (hasVerifiedNemoModel(model)) {
      toast(`${NEMO_MODELS[model]?.short ?? model} loaded`);
      onDone?.(true);
      return null;
    }
    return runJob({
  ...
  function installRemoteModel({ backend, model, label, onDone } = {}) {
    const runtime = managedSttRuntime(backend);
    if (!runtime) {
      toast(`No managed installer exists for ${backend || "this STT backend"}`, "warning");
      return null;
    }
    const support = managedRuntimeSupport();
    if (!support.supported) {
      toast(support.message, "warning");
      return null;
    }
    // Keep one job per backend: Qwen and Fun-ASR can install concurrently,
    // while two models of the same managed runtime still serialize.
    const key = `runtime:${backend}`;
    const existing = jobs.get(key);
    if (existing?.state === "running" && existing.model !== model) {
      toast(`${existing.label} is already installing - wait for it to finish`, "warning");
      return null;
    }
    let command;
    const modelCached = managedSttModelCached(backend, model);
    try {
      command = buildManagedSttInstallCommand({ backend, model, modelCached });
    } catch (err) {
    ...
    return runJob({
      key,
      label: modelCached ? `Loading ${label || model}` : label || model,
      kind: "runtime",
      model,
      backend,
      modelCached,
  ...
  function stopRemoteModel({ backend } = {}) {
    const runtime = managedSttRuntime(backend);
    if (!runtime) return null;
    const installing = jobs.get(`runtime:${backend}`);
    if (installing?.state === "running") cancelJob(installing);
    let command;
    try {
      command = buildManagedSttStopCommand({ backend });
    } catch (err) {
      toast(err.message, "warning");
      return null;
    }
    return runJob({
      key: `runtime-stop:${backend}`,
      label: `${runtime.id} runtime shutdown`,
      kind: "runtime-stop",
      backend,
      command,
    });
  ...
  function stepSox({ next, back }) {
    showSoxMissingDialog(api, toast, {
      title: guidedRun
        ? `${guidedText("setup")} · sox is not installed`
        : "Setup · sox is not installed (recording needs it)",
      onContinue: next,
      onFound: next,
      back,
    });
  ...
  function stepInstall({ back, next } = {}) {
    const options = [
      {
        ...
        onSelect() {
          ...
          runJob({
            ...
            onDone(ok) {
              ...
              downloadModel(model, { onDone: next });
            },
          });
        },
      },
  ...
  function run({ back } = {}) {
    ...
    guidedRun = true;
    guidedLocale = normalizeUiLocale(kv.get("voice.locale", kv.get("stt.language", "en")));
    guidedLanguage = kv.get("stt.language", guidedLocale) || guidedLocale;
    guidedMode = getVoiceMode?.() || kv.get("voice.mode", "hold");
    stepGuidedLanguage({ back });
  }

  function runEngineSteps({ back, next = () => stepMode({ back }) } = {}) {
    if (!hasBinary("sox")) {
      const showSox = () => stepSox({ next: () => runEngineSteps({ back, next }), back });
      showSox();
      return;
    }
    const config = resolveSttConfig(kv, opts);
    if (config.backend !== STT_BACKENDS.NEMO) {
      if (!managedSttRuntime(config.backend)) {
        toast("Select a managed transcription model in /voice", "warning");
        back?.();
        return;
      }
      if (!managedSttRuntimeInstalled(config.backend, config.model)) {
        installRemoteRuntime?.({ next: () => setupCleanup(back, next), back });
        return;
      }
      setupCleanup(back, next);
      return;
    }
    if (!hasBinary("nemo-speech")) {
      stepInstall({ back, next: () => setupCleanup(back, next) });
      return;
    }
    ...
    if (!hasVerifiedNemoModel(model)) {
      const showPrewarm = () =>
        stepPrewarm({ model, next: () => setupCleanup(showPrewarm, next), back });
      showPrewarm();
      return;
    }
    setupCleanup(back, next);
  ...
  function setupCleanup(rootBack, next = () => stepMode({ back: rootBack })) {
    ...
    // First-run setup chooses the host's small model automatically rather than
    // inserting a second, advanced model browser after STT setup.
    if (guidedRun) {
      const mode = kv.get("cleanup.mode");
      if (mode === "skip" || (mode === "opencode" && kv.get("cleanup.model"))) {
        next();
        return;
      }
      const stillCurrent = menu.guard();
      resolveOpencodeCleanup(client, logger, { state })
        .then((resolved) => {
          if (!stillCurrent()) return;
          configureGuidedCleanup(kv, resolved);
          next();
        })
        .catch((error) => {
          if (!stillCurrent()) return;
          logger?.log("STT", `Cleanup setup failed: ${error.message}`, "debug");
          kv.set("cleanup.mode", "skip");
          next();
        });
      return;
    ...
    const showCleanup = () =>
      stepCleanup({
        title: guidedRun ? guidedText("cleanup") : undefined,
        back: rootBack,
        next,
      });
    showCleanup();
  ...
  return {
    ...
    installingRemoteModels: () =>
      new Set(
        activeJobs()
          .filter((job) => job.kind === "runtime")
          .map((job) => `${job.backend}:${job.model}`),
      ),
  };
}

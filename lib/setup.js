...
export function createVoiceSetup(ctx) {
  ...
  async function runCleanupAutoPick({ next, back, fresh = false } = {}) {
    ...
    // Zen models probe through session chat, so they work even when /v1 is
    // auth-gated; only bail when there is nothing to probe on any transport.
    const zen = small.filter((m) => m.id.startsWith("opencode/"));
    if (small.length === 0 || (!baseUrl && zen.length === 0)) {
    ...
    if (stale.length > 0) {
      ...
      // Zen rows via session chat (they 429 on gated /v1 hosts), the rest
      // via the OpenAI-compat endpoint, merged back in input order.
      const [http, viaSession] = await Promise.all([
        baseUrl
          ? probeCleanupModels(
              baseUrl,
              stale.filter((m) => !m.id.startsWith("opencode/")),
              logger,
            )
          : Promise.resolve([]),
        probeZenModelsViaSessions(client, stale, logger),
      ]);
      const byId = new Map([...http, ...viaSession].map((r) => [r.id, r]));
      probed.push(...stale.map((m) => byId.get(m.id)).filter(Boolean));

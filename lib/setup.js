 ...
 * `checkSox` is injectable so tests do not probe the real PATH.
 */
export function showSoxMissingDialog(
  ...
  menu({
    title,
    options: [
      // Disabled row: the command itself, readable for as long as it takes to
      // type it into another terminal.
      { title: soxInstallCommand(), value: "command", disabled: true },
      {
...
export function createVoiceSetup(ctx) {
  ...
  async function runCleanupAutoPick({ next, back, fresh = false } = {}) {
    ...
    // Failed models stay on screen, unselectable, showing why they failed:
    // HTTP 429 is the host's rate limit, 401/403 means reauth, 404 means the
    // catalog listed a model this server does not serve. Dropping them would
    // read as the model list itself being broken.

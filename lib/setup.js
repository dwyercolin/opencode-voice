...
export function createVoiceSetup(ctx) {
  ...
  function jobsDialog({ back } = {}) {
    ...
    options.push(
      back
        ? {
            title: "Back",
            description: "everything here continues in the background",
            value: "back",
            onSelect() {
              back();
            },
    ...
    dialog(running > 0 ? `Voice · ${running} in progress` : "Voice · recent jobs", options);
  }
  ...
  function stepPrewarm({ model: requested, next, title } = {}) {
    ...
    dialog(title ?? `Setup · ${name} is not downloaded yet`, [
      {
  ...
  function probeRunningDialog({ next, back } = {}) {
    ...
    if (back) {
      options.push({
        title: "Back",
        description: "testing continues in the background",
        value: "back",
        onSelect() {
          leave();
          back();
    ...
    dialog(probeTestingTitle(run), options);
  }
  ...
  function runEngineSteps() {
    ...
    if (!hasVerifiedNemoModel(model)) {
      stepPrewarm({ model, next: stepCleanup });
      return;
    }
    stepCleanup();
  }

...
export function createVoiceSetup(ctx) {
  ...
  function stepPrewarm() {
    const model = kv.get("stt.nemo.model") || DEFAULT_NEMO_MODEL;
    dialog("Setup · the transcription model is not downloaded yet", [
      {
        title: "Download it for me",
        description: "~700 MB in the background, keep working",
        value: "pull",
        onSelect() {
          runInstall("Downloading transcription model", `nemo-speech model pull ${model}`, {
            parseProgress: true,
          });
        },
      },
      {
        title: "Not now",
        description: "the first dictation will download it instead",
        value: "cancel",
        onSelect() {
          api.ui.dialog.clear();
        },

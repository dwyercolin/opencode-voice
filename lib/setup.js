...
import { STT_BACKENDS, resolveSttConfig, validSttEndpoint } from "./stt-engines.js";
import {
...
export function createVoiceSetup(ctx) {
  ...
  // routes over HTTP.
  const { api, kv, client, state, logger, opts, toast, getTalkKey, setVoiceMode, configureRemote } =
    ctx;
  ...
  function runJob({ key, label, command, kind = "install", model, backend, onDone }) {
    const existing = jobs.get(key);
    ...
    if (kind === "runtime") {
      const progress = managedSttRuntimeProgress(backend);
      if (progress) {
  ...
  function installRemoteModel({ backend, model, label, onDone } = {}) {
    ...
    let command;
    try {
      command = buildManagedSttInstallCommand({ backend, model });
    } catch (err) {
  ...
  function runEngineSteps({ back } = {}) {
    ...
    if (config.backend !== STT_BACKENDS.NEMO) {
      if (!validSttEndpoint(config.endpoint, config.backend)) {
        configureRemote?.({ next: () => setupCleanup(back), back });
        return;
      }
      // Remote runtimes own their model lifecycle. The picker verifies the
      // endpoint before use; setup must not install NeMo behind the user's

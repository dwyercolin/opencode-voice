...
export function createVoiceSetup(ctx) {
  ...
  async function runCleanupAutoPick({ next, back, fresh = false } = {}) {
    ...
    if (working.length === 0) {
      // "None responded" is useless without a reason: HTTP 429 means the
      // active subscription's rate limit gates the whole endpoint (observed
      // with Claude Max: even Zen free models 429 through the host server
      // while the plan is throttled), HTTP 401/403 means the provider needs
      // (re)auth in opencode, HTTP 404 means the catalog lists a model the
      // host server does not serve, timeouts mean a slow endpoint, and
      // "fetch failed" usually means stale server discovery.

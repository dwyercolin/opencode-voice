...
export function createVoiceSetup(ctx) {
  ...
  async function runCleanupAutoPick({ next, back, fresh = false } = {}) {
    ...
    if (stale.length > 0) {
      ...
      // All probes go through session chat - the server's real per-provider
      // routing - because /v1 is auth-gated on some hosts and 429s everything.
      probed.push(...(await probeModelsViaSessions(client, stale, logger)));

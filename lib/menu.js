...
//
// opencode's DialogSelect renders each option's title, muted description, and
// right-aligned footer on one line with `overflow: hidden` and no wrapping.
// Anything past the panel width is silently clipped mid-word, so rows have to
// be budgeted against the real column count instead of hoping the copy is short
// enough.
...
export const BACK_KEY = "alt+left";

const BACK_COMMAND = "opencode-voice.dialog.back";
const navigationByApi = new WeakMap();
...
/** Display columns a string occupies in the terminal, not its UTF-16 length. */
export function displayWidth(value) {
  let width = 0;
  for (const char of String(value ?? "")) {
    const code = char.codePointAt(0);
    if (code <= 0x1f || (code >= 0x7f && code <= 0x9f) || ZERO_WIDTH.test(char)) continue;
...
export function fitRows(options, width) {
  ...
  const titleWidth = Math.min(
    Math.max(0, ...described.map(displayWidth)),
  ...
  return rows.map((o, i) => {
    ...
    const title = `${titles[i]}${" ".repeat(Math.max(0, titleWidth - displayWidth(titles[i])))}`;
    if (!o.description) return { ...o, title };
    // The footer is right-aligned in the same row, so it costs space too.
    const footer = o.footer ? displayWidth(o.footer) + 2 : 0;
    const room = width - displayWidth(title) - 1 - footer;
...
/**
 * The row that leads out of a sub-screen without changing anything. Every
 * picker reached from another screen gets one, matching the Alt+Left shortcut.
 * Escape still closes the whole dialog. Appended in place and returned for
 * chaining; a screen with no parent passes no `back` and keeps its list unchanged.
 */
export function addBackRow(options, back, description = "nothing changed") {
...
function navigationState(api) {
  let state = navigationByApi.get(api);
  if (state) return state;

  state = { back: null, screen: null, request: 0 };
  navigationByApi.set(api, state);
  const dispose = api.keymap?.registerLayer?.({
    enabled: () => Boolean(state.back) && api.ui.dialog.open !== false,
    commands: [
      {
        name: BACK_COMMAND,
        title: "Back",
    ...
    bindings: [{ key: BACK_KEY, cmd: BACK_COMMAND, desc: "Back" }],
  });
...
export function createMenu(api) {
  ...
  const navigation = navigationState(api);
  function menu({
    title,
    options,
    size = "large",
    ...
    const screen = Symbol(title);
    navigation.request++;
    navigation.screen = screen;
    navigation.back = back ?? null;
    api.ui.dialog.replace(
      () =>
        api.ui.DialogSelect({
          ...
          options: rows,
          current,
          placeholder: placeholder ?? (filter ? "Search…" : "Filter options…"),
          flat: true,
          skipFilter: !filter,
        }),
      () => {
        onClose?.();
        if (navigation.screen !== screen) return;
        navigation.request++;
        navigation.screen = null;
  ...
  // Async screens capture this before loading. Any close, replacement, or
  // newer request invalidates the callback so stale results cannot reopen UI.
  menu.guard = () => {
    const request = ++navigation.request;
    const screen = navigation.screen;
    return () => navigation.request === request && navigation.screen === screen;
  };
  return menu;
}

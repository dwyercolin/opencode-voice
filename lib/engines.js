...
// SHA-256 check. Layout is the CLI's own; only the marker suffix is relied on.
const NEMO_CACHE_DIR = path.join(os.homedir(), ".cache", "nemo-speech", "models");

/**
 * Whether at least one downloaded model has passed verification. False is what
 * a fresh install looks like - and what makes the first transcription quietly
 * turn into a ~700 MB download.
 */
export function hasVerifiedNemoModel(cacheDir = NEMO_CACHE_DIR) {
  let entries = [];
  try {
  ...
  return entries.some((entry) => entry.endsWith(".gguf.verified"));
}

/** Transcription budget: generous on a first run (download inside the call), tight otherwise. */
...
export function transcribeNemo({ wavFile, model = "", logger }) {
  return new Promise((resolve) => {
    ...
    // First use downloads the pinned GGUF (~700 MB) inside this very call, so
    // the budget depends on whether a verified model is already in the cache;
    // a normal local transcription finishes well inside 3 minutes.
    const timeoutMs = transcribeTimeoutMs(hasVerifiedNemoModel());
    const timer = setTimeout(() => {
      ...
      resolve({
        error: `Transcription timed out (${timeoutMs / 1000}s - first run downloads the ~700 MB model; try again once it lands)`,
      });
    }, timeoutMs);

...
export function hasBinary(name) {
  ...
  for (const flag of ["--version", "--help"]) {
    // spawnSync rather than execSync: the resolved command may be an absolute
    // path with spaces in it, which must not be spliced into a shell string.
    const r = spawnSync(binaryCommand(name), [flag], { stdio: "ignore", timeout: 5000 });
    if (r.status === 0) {
      found = true;
      break;
    }

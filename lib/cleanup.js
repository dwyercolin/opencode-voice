...
// ---- Session-transport cleanup (Zen models) ----
//
// Some hosts gate the OpenAI-compat /v1 endpoint behind the active
// subscription's auth - a throttled Claude Max plan 429s every model there,
// Zen free ones included, because the model parameter is never consulted.
// The server's own chat routing (session.prompt, what the TUI uses) does
// proper per-provider routing, so Zen ("opencode/*") models travel that way
// instead. Each call runs in a throwaway session so context never grows.
...
// Session path params differ across SDK generations: the published types
// name it "id", the SDK bundled in current TUI builds names it "sessionID"
// (proven by a live run leaving the literal "%7BsessionID%7D" placeholder in
// the URL). Send both; the client picks the one its template knows and
// ignores the other.
...
async function withTempSession(client, fn) {
  const created = await client.session.create({ body: { title: TEMP_SESSION_TITLE } });
  ...
  try {
    ...
    try {
      await client.session.delete({ path: sessionPath(id) });
    } catch {}
  }
}
...
/** Is this a Zen model id, routable through session chat? */
export function isZenModel(id) {
  return String(id || "").startsWith("opencode/");
}

/**
 * One cleanup pass through the server's own chat routing: create a
 * throwaway session, prompt with the given system override, delete it.
 * Returns { text } or { text: null, error } like the /v1 client does.
 */
export async function runCleanupViaSession(client, model, system, prompt, logger) {
...
/**
 * Probe each Zen model with one tiny cleanup request through session chat
 * (mirrors probeCleanupModels, which probes /v1). Returns
 * [{ id, name, ok, ms?, fixes?, error? }] in input order.
 */
export async function probeZenModelsViaSessions(client, models, logger, { limit = 6 } = {}) {
  const candidates = (models ?? []).filter((m) => m?.id && isZenModel(m.id)).slice(0, limit);
  return Promise.all(
    candidates.map(async (m) => {
      try {
        return await withTempSession(client, async (id) => {
          ...
          const result = await client.session.prompt({
            ...
            body: {
              ...
              parts: [
                {
                  type: "text",
                  text: `Clean up this speech-to-text transcription:\n\n${PROBE_INPUT}`,
                },
              ],
            },
          });
        ...
        return { id: m.id, name: m.name, ok: false, error: err.message.slice(0, 80) };
      }
    }),
  );
}

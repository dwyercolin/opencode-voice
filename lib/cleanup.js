 ...
 * rankProbeResults.
 *
 * `onResult` fires as each model answers. The whole run takes tens of
 * seconds - it is a real request per model, a few at a time - so the caller
 * needs the answers as they land to show honest progress rather than a
 * frozen screen until the last one returns.
 */
export async function probeCleanupModels(
  client,
  models,
  logger,
  { limit = 8, concurrency = 3, timeoutMs = 25000, onResult } = {},
) {
  ...
  return mapWithConcurrency(candidates, concurrency, async (m) => {
    ...
    const probe = result.text
      ...
      : { id: m.id, name: m.name, ok: false, error: String(result.error).slice(0, 90) };
    // Never let a listener's throw take the whole probe run down with it.
    try {
      onResult?.(probe);
    } catch {}
    return probe;

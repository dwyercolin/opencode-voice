...
import { createVoiceSetup, shortModelLabel } from "./setup.js";
...
export function registerSTT(api, kv, complete, opts, logger) {
  ...
  function setVoiceMode(mode, { silent = false } = {}) {
    ...
    if (silent) return;
    if (mode === "tap") {
      toast(`Voice mode: tap - press ${talkKey} to start, press again to finish`);
    } else {
  ...
  function cleanupSummary() {
    if (opts?.endpoint) return shortModelLabel(opts.model || opts.endpoint);
    ...
    return model ? shortModelLabel(model) : "opencode default model";
  }
    ...
    api.ui.dialog.replace(() =>
      api.ui.DialogSelect({
        ...
        options: [
          {
            title: `Input mode: ${
              mode === "tap" ? `tap ${talkKey} to start and finish` : `hold ${talkKey} to talk`
            }`,
            value: "mode",
            onSelect() {
          ...
          {
            title: `Auto-submit when dictation ends: ${autoSubmitEnabled() ? "on" : "off"}`,
            value: "autosubmit",
            onSelect() {
          ...
          {
            title: `Text cleanup: ${cleanupSummary()}`,
            value: "cleanup",
            onSelect() {
              if (opts?.endpoint) {
          ...
          {
            title: `Transcription model: ${NEMO_MODELS[getNemoModel(kv)]?.short ?? "default"}`,
            value: "model",
            onSelect() {
          ...
          {
            title: `Microphone: ${kv.get("stt.mic", "") || "system default"}`,
            value: "mic",
            onSelect() {
          ...
          {
            title: `Auto-gain for quiet input: ${autoGainEnabled(kv) ? "on" : "off"}`,
            value: "gain",
            onSelect() {

// Speech-to-text: sox recording, pluggable local/remote transcription, LLM
// cleanup, and Claude Code-style voice input modes (/voice).
...
// after every stash pop. User prompt edits invalidate this tracking so a later
// stash preserves the edited content instead of resurrecting deleted text.
...
function normalizeLiveCharacters(text) {
  return Array.from(String(text || ""))
    .filter((char) => /[\p{L}\p{N}]/u.test(char))
    .join("");
}

function hasCharacterTranscriptScript(text) {
  return /[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}]/u.test(String(text || ""));
}

function joinTranscriptSegments(previousText, nextText, characterOverlap) {
  if (!previousText) return nextText;
  if (!nextText) return previousText;
  const combined = `${previousText}${nextText}`;
  const noSeparator = characterOverlap && !/\s/u.test(combined) && !/[A-Za-z0-9]/u.test(combined);
  return noSeparator ? combined : `${previousText} ${nextText}`.trim();
}

/**
 * Remove a conservative character-level overlap for languages whose transcript
 * has no whitespace boundaries, such as Chinese. Punctuation and spaces are
 * ignored while matching, but the returned cut preserves the tail's original
 * punctuation and spacing.
 */
export function stripOverlappingCharacters(previousText, tailText, maxWindow = 80) {
  if (!previousText || !tailText) return tailText;
  const previous = normalizeLiveCharacters(previousText);
  const tail = normalizeLiveCharacters(tailText);
  const maxK = Math.min(previous.length, tail.length, maxWindow);
  let best = 0;
  for (let k = maxK; k >= 2; k--) {
    if (previous.slice(-k) === tail.slice(0, k)) {
      best = k;
      break;
    }
  }
  if (!best) return tailText;

  let matched = 0;
  let cut = 0;
  for (const char of Array.from(tailText)) {
    cut += char.length;
    if (/[\p{L}\p{N}]/u.test(char)) matched++;
    if (matched === best) break;
  }
  return tailText.slice(cut).trim();
}
...
// Detect prompt edits without treating the configured voice key as a prompt
// edit. This lets the next stash become the source of truth after a user
// changes or deletes text that the plugin previously appended.
export function isPromptEditingKey(event, voiceKey = "") {
  ...
  const key = [...modifiers, name].join("+");
  if (voiceKey && key === String(voiceKey).trim().toLowerCase()) return false;

  if (["backspace", "delete", "enter", "return", "tab", "space"].includes(name)) return true;
  if (
    !event.ctrl &&
    !event.alt &&
    !event.meta &&
  ...
  if (event.ctrl || event.meta || event.super || event.hyper) {
    return ["v", "x", "y", "k", "u", "w"].includes(name);
  }
  return false;
}
...
function getSttConfig(kv) {
  return resolveSttConfig(kv, pluginOpts);
}
...
async function transcribeWithEngine(kv, logger, wavFile = path.join(tmpDir, WAV_FILENAME)) {
  ...
  const config = getSttConfig(kv);
  const readiness = sttReadiness(config);
  if (!readiness.ready) return { error: readiness.message };
  return await transcribeWithBackend({
    config,
    wavFile,
    timeoutMs: Number(pluginOpts.sttTimeoutMs) || 30000,
    logger,
  });
}
...
async function collectFinalTranscript(kv, logger, toast) {
  ...
  const characterOverlap =
    config.merge === "characters" ||
    (config.merge === "auto" && hasCharacterTranscriptScript(`${liveText || ""}${tail.text}`));
  const deduped = characterOverlap
    ? stripOverlappingCharacters(liveText, tail.text)
    : stripOverlappingWords(liveText, tail.text);
  debugLog(
    `pipeline: tail dedup ${tail.text.length} chars -> ${deduped.length} chars: ${JSON.stringify(deduped.slice(0, 60))}`,
  );
  const combined = joinTranscriptSegments(liveText, deduped, characterOverlap);
...
export function registerSTT(api, kv, complete, opts, logger) {
  ...
  if (typeof api.keymap?.intercept === "function") {
    const disposePromptEditCapture = api.keymap.intercept(
      "key",
      ({ event }) => {
        if (!pluginPromptText || recording || processing) return;
        if (!isPromptEditingKey(event, talkKey)) return;
        pluginPromptText = null;
        debugLog("prompt input changed after dictation - discarded accumulated plugin text");
      },
      { priority: -100 },
    );
    api.lifecycle?.onDispose?.(disposePromptEditCapture);
  }
  ...
  function showSttEndpointDialog({ next, back } = {}) {
    if (typeof api.ui.DialogPrompt !== "function") {
      toast("This OpenCode version cannot configure an STT endpoint", "warning");
      return;
    }
    const config = getSttConfig(kv);
    const websocket = config.backend === STT_BACKENDS.FUNASR_WEBSOCKET;
    const current = config.endpoint;
    let accepted = false;
    api.ui.dialog.replace(
      () =>
        api.ui.DialogPrompt({
          title: websocket ? "Fun-ASR WebSocket endpoint" : "STT server endpoint",
          description: () =>
            websocket
              ? "Fun-ASR URL; server model must match the selected checkpoint"
              : "OpenAI-compatible base URL, e.g. http://127.0.0.1:8000/v1",
          value: current,
          placeholder: websocket ? "ws://127.0.0.1:10095" : "http://127.0.0.1:8000/v1",
          onConfirm(value) {
            const endpoint = String(value || "").trim();
            if (!validSttEndpoint(endpoint, config.backend)) {
              toast(
                websocket
                  ? "Enter a ws:// or wss:// Fun-ASR endpoint"
                  : "Enter an http:// or https:// STT endpoint",
                "warning",
              );
              return;
            }
            accepted = true;
            remoteInstallIntent = null;
            const managedBackend = kv.get("stt.runtime.backend", "");
            if (managedBackend) setup.stopRemoteModel({ backend: managedBackend });
            kv.set("stt.endpoint", endpoint);
            kv.set("stt.runtime.backend", "");
            kv.set("stt.runtime.model", "");
            api.ui.dialog.clear();
            next?.();
          },
          onCancel() {
  ...
  let remoteInstallIntent = null;

  function showSttRuntimeChoice({ next, back, waitForInstall = false } = {}) {
    const config = getSttConfig(kv);
    const runtime = managedSttRuntime(config.backend);
    if (!runtime) {
      showSttEndpointDialog({ next, back });
      return;
    }
    const install = () => {
      api.ui.dialog.clear();
      remoteInstallIntent = { backend: config.backend, model: config.model };
      const clearStaleManagedRuntime = () => {
        if (kv.get("stt.runtime.backend", "") !== config.backend) return;
        kv.set("stt.endpoint", "");
        kv.set("stt.runtime.backend", "");
        kv.set("stt.runtime.model", "");
        remoteInstallIntent = null;
      };
      const job = setup.installRemoteModel({
        backend: config.backend,
        model: config.model,
        label: config.displayName,
        onDone(ok) {
          const current = getSttConfig(kv);
          const ownsInstallIntent =
            remoteInstallIntent?.backend === config.backend &&
            remoteInstallIntent?.model === config.model;
          if (!ok) {
            if (
              ...
              clearStaleManagedRuntime();
            }
            if (ownsInstallIntent) remoteInstallIntent = null;
            return;
          }
          if (
            !ownsInstallIntent ||
            ...
            if (current.backend === config.backend) clearStaleManagedRuntime();
            if (ownsInstallIntent) remoteInstallIntent = null;
            return;
          }
          kv.set("stt.endpoint", managedSttEndpoint(config.backend));
          kv.set("stt.runtime.backend", config.backend);
          kv.set("stt.runtime.model", config.model);
          remoteInstallIntent = null;
          toast(`${config.displayName} ready at ${managedSttEndpoint(config.backend)}`, "success");
          if (waitForInstall) next?.();
        },
      });
      if (!job) {
        clearStaleManagedRuntime();
    ...
    const options = [
      {
        ...
        description:
          runtime.mode === "executable"
            ? "download the native runtime and model"
            : "set up the GPU runtime and download the model",
        value: "install",
        onSelect: install,
      },
      ...(runtime.mode === "executable"
        ...
        : [
            {
              ...
              description: "enter an already running compatible endpoint",
              value: "external",
              onSelect() {
                showSttEndpointDialog({ next, back });
              },
    ...
    addBackRow(options, back, "keep the current transcription model");
    menu({
      title: `${config.displayName} setup`,
      options,
      back,
    });
  }

  function showSttApiKeyEnvDialog({ next, back } = {}) {
    if (typeof api.ui.DialogPrompt !== "function") {
      toast("This OpenCode version cannot configure STT authentication", "warning");
      return;
    }
    const current = getSttConfig(kv).apiKeyEnv;
    let accepted = false;
    api.ui.dialog.replace(
      () =>
        api.ui.DialogPrompt({
          title: "STT API key environment variable",
          description: () => "Optional; stores only the variable name, never the secret",
          value: current,
          placeholder: "OPENAI_API_KEY",
          onConfirm(value) {
            accepted = true;
            kv.set("stt.apiKeyEnv", String(value || "").trim());
            api.ui.dialog.clear();
            next?.();
          },
          onCancel() {
  ...
  function showModelMissing() {
    ...
    if (config.backend !== STT_BACKENDS.NEMO) {
      const key = `${config.backend}:${config.model}`;
      if (setup.installingRemoteModels().has(key)) {
        toast(`${config.displayName} is still installing - dictation works once it lands`);
        return;
      }
      showSttRuntimeChoice({ back: showVoiceMenu });
      return;
    }
  ...
  function handleTalkPress() {
    ...
    const config = getSttConfig(kv);
    const runtimeInstalling = setup
      .installingRemoteModels()
      .has(`${config.backend}:${config.model}`);
    if (!recording && (runtimeInstalling || !sttReadiness(config).ready)) {
      showModelMissing();
      return;
    }
    if (getVoiceMode() === "tap") {
  ...
  const setup = createVoiceSetup({
    ...
    getTalkKey: () => talkKey,
    setVoiceMode,
    configureRemote: ({ next, back } = {}) =>
      showSttRuntimeChoice({ next, back, waitForInstall: true }),
  });
  ...
  // Local models and managed remote runtimes report their state in the footer;
  // external remote servers report endpoint configuration there instead.
  function currentManagedRuntimeReady(config) {
    return (
      managedSttRuntime(config.backend) &&
      config.endpoint === managedSttEndpoint(config.backend) &&
      kv.get("stt.runtime.backend", "") === config.backend &&
      managedSttRuntimeInstalled(config.backend, config.model)
    );
  }

  function currentModelStatus(config) {
    return currentManagedRuntimeReady(config) ? "ready" : sttModelStatus(config);
  }

  function modelRowState(key) {
    const option = sttModelOption(key);
    if (!option) return "unknown";
    if (option.backend === STT_BACKENDS.NEMO && setup.downloadingModels().has(option.model)) {
      return "downloading";
    }
    if (setup.installingRemoteModels().has(`${option.backend}:${option.model}`)) {
      return "installing";
    }
    if (
      managedSttRuntime(option.backend) &&
      managedSttRuntimeInstalled(option.backend, option.model)
    ) {
      return "ready";
    }
    return sttModelStatus({
      ...
      model: option.model,
      endpoint: getSttConfig(kv).endpoint,
    });
  }
  ...
  function modelSummary() {
    const config = getSttConfig(kv);
    const name = config.displayName || config.model || "default";
    if (setup.installingRemoteModels().has(`${config.backend}:${config.model}`)) {
      return `${name} · installing…`;
    }
    const status = currentModelStatus(config);
    if (status === "ready" || status === "configured") return name;
    if (status === "downloading") return `${name} · downloading…`;
    if (status === "partial") return `${name} · part-downloaded`;
    if (status === "endpoint-required") return `${name} · endpoint required`;
    return `${name} · not ready`;
  }

  function showModelDialog({ next, back } = {}) {
    const options = Object.entries(STT_MODELS).map(([key, v]) => {
                  ...
                  : status === "configured"
                    ...
                    : status === "endpoint-required"
                      ...
                      : status === "backend-missing"
                        ? "nemo-speech missing · run setup"
                        : v.backend === STT_BACKENDS.NEMO
                          ? "not downloaded · enter downloads"
                          : "not configured · enter setup",
        value: key,
        onSelect() {
          kv.set("stt.model", key);
          kv.set("stt.backend", v.backend);
          if (v.backend === STT_BACKENDS.NEMO) {
            kv.set("stt.nemo.model", v.model);
            // Start the download HERE rather than letting it happen inside the
            // first dictation, where a model switch would swallow a recording.
            if (status !== "ready" && status !== "downloading") setup.downloadModel(v.model);
          } else {
            kv.set("stt.remote.model", v.model);
            const endpoint = getSttConfig(kv).endpoint;
            if (!validSttEndpoint(endpoint, v.backend)) {
              showSttRuntimeChoice({
                next,
                back: () => showModelDialog({ next, back }),
              });
              return;
            }
            if (
              endpoint === managedSttEndpoint(v.backend) &&
              kv.get("stt.runtime.backend", "") === v.backend &&
              !managedSttRuntimeInstalled(v.backend, v.model)
            ) {
              showSttRuntimeChoice({
                next,
                back: () => showModelDialog({ next, back }),
  ...
  function showVoiceMenu() {
    ...
    const options = [
      ...
      ...([STT_BACKENDS.OPENAI_COMPATIBLE, STT_BACKENDS.FUNASR_WEBSOCKET].includes(
        getSttConfig(kv).backend,
      )
        ? [
            {
              title:
                getSttConfig(kv).backend === STT_BACKENDS.FUNASR_WEBSOCKET
                  ? "Fun-ASR WebSocket endpoint"
                  : "STT server endpoint",
              description: getSttConfig(kv).endpoint || "not configured",
              footer:
                getSttConfig(kv).backend === STT_BACKENDS.FUNASR_WEBSOCKET
                  ? "Fun-ASR WebSocket"
              ...
              value: "stt.endpoint",
              onSelect() {
                showSttEndpointDialog({ next: showVoiceMenu, back: showVoiceMenu });
              },
            },
            ...(getSttConfig(kv).backend === STT_BACKENDS.OPENAI_COMPATIBLE
              ? [
                  {
                    title: "STT API key environment variable",
                    description: getSttConfig(kv).apiKeyEnv || "none",
                    footer: "optional · secret stays in environment",
                    value: "stt.api-key",
                    onSelect() {
                      showSttApiKeyEnvDialog({ next: showVoiceMenu, back: showVoiceMenu });
                    },
                  },
                ]
              : []),
  ...
  return [
    ...
    },
    {
      ...
      value: "stt.model",
      description: "Choose a local or OpenAI-compatible transcription model",

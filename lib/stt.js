...
function startRecording(kv, backend, toast, logger, trimSilence = true) {
  ...
  if (!hasBinary("sox")) {
    ...
    toast(`sox is not installed - recording needs it. Install: ${soxInstallCommand()}`, "error");
    return;
  }
  ...
  soxProc.on("error", (err) => {
    ...
    if (recording) {
      ...
      toast(
        err.code === "ENOENT"
          ? `sox is not installed - recording needs it. Install: ${soxInstallCommand()}`
          : `Recording failed: ${err.message}${audioFailureSuffix(backend)}`,
        "error",
      );
    }
  });

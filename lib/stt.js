// Speech-to-text: sox recording, engine dispatch (whisper.cpp with VAD,
// nemo-speech, or a transcription API), LLM normalization, and Claude
// Code-style voice input modes (/voice hold|tap|off).
...
// File-based debug log: opencode's app logger filters debug output, so voice
// diagnostics land here to make field issues diagnosable.
...
// late partial can never interleave with the final swap (which previously
// produced duplicated text that got submitted).
...
// can capture stale state - the cause of "previous dictation vanished".
// Stashing waits for STASH_SETTLE_MS after the last mutation.
...
// Default engine: nemo-speech when its binary is installed, else whisper
// (or the transcription API when configured and nothing local is available).
function resolveEngine(kv) {
  const configured = pluginOpts.sttEngine;
  const fallback =
    configured === "nemo" || configured === "whisper" || configured === "api"
      ? configured
      : hasBinary("nemo-speech")
        ? "nemo"
        : hasBinary("whisper-cli") || !sttApiEndpoint
          ? "whisper"
          : "api";
  const engine = kv.get("stt.engine", fallback);
  return engine === "nemo" || engine === "whisper" || engine === "api" ? engine : fallback;
}

function getWhisperModelName(kv) {
  const model = kv.get("stt.model", DEFAULT_WHISPER_MODEL);
  return WHISPER_MODELS[model] ? model : DEFAULT_WHISPER_MODEL;
}

function getWhisperModelPath(kv) {
  const dir = WHISPER_MODELS_DIRS.find((d) => fs.existsSync(d)) || WHISPER_MODELS_DIRS[0];
  return path.join(dir, WHISPER_MODELS[getWhisperModelName(kv)].file);
}
...
async function transcribeWithEngine(kv, logger, wavFile = path.join(tmpDir, WAV_FILENAME)) {
  ...
  const engine = resolveEngine(kv);
  logger?.log("STT", `Transcribing with engine=${engine}`, "debug");
  if (engine === "api") {
    return await transcribeApi({
      endpoint: sttApiEndpoint,
      model: kv.get("stt.api.model") || sttApiModel,
  ...
  if (engine === "nemo") {
    if (!hasBinary("nemo-speech")) {
      return {
        error:
          "nemo-speech not found on PATH - install it (github.com/NVIDIA/NeMo-Speech.cpp) or pick another engine via /stt-engine",
      };
    }
    return await transcribeNemo({ wavFile, model: getNemoModel(kv), logger });
  }

  const vadModel = pluginOpts.sttVad === false ? null : findVadModel();
  return await transcribeWhisper({
    modelPath: getWhisperModelPath(kv),
    wavFile,
    language: getLanguage(kv),
    vadModel,
    logger,
  });
}
...
async function collectFinalTranscript(kv, logger, toast) {
  ...
  if (tail.error) {
    ...
    return { text: liveText || "", vadFallback: tail.vadFallback };
  }
  if (!tail.text) {
    return { text: liveText || "", vadFallback: tail.vadFallback };
  }
  ...
  return { text: combined, vadFallback: tail.vadFallback };
}
...
async function doTranscribePipeline(
  ...
  try {
    ...
    if (result.vadFallback) {
      logger?.log("STT", "whisper-cli lacks VAD support; silence filtering disabled", "warn");
      toast("whisper-cli too old for VAD - silence filtering disabled this run", "warning");
    }
    if (!result.text) {
...
export function registerSTT(api, kv, complete, opts, logger) {
  ...
  // One-time probe: what the host client exposes, for defaulting the cleanup
  // endpoint to the opencode server without user configuration.
  (async () => {
    try {
      const keys = Object.keys(client ?? {});
      debugLog(`probe: client keys=[${keys.join(",")}]`);
      for (const k of keys) {
        try {
          const v = client[k];
          if (typeof v === "string" && /^https?:\/\//.test(v)) {
            debugLog(`probe: client.${k} = ${v}`);
          } else if (v && typeof v === "object" && typeof v.baseUrl === "string") {
            debugLog(`probe: client.${k}.baseUrl = ${v.baseUrl}`);
          }
        } catch {}
      }
      if (typeof client.config?.get === "function") {
        ...
        const interesting = {};
        for (const k of ["model", "small_model", "default_agent"]) {
          if (cfg?.[k] != null) interesting[k] = cfg[k];
        }
        debugLog(`probe: config.get -> ${JSON.stringify(interesting)}`);
      } else {
        debugLog("probe: client.config.get unavailable");
      }
      if (typeof client.config?.providers === "function") {
        const provs = await client.config.providers();
        const summary = (provs?.providers || []).map((p) => `${p.id}:${(p.models || []).length}`);
        debugLog(`probe: providers -> ${JSON.stringify(summary).slice(0, 300)}`);
        debugLog(`probe: provider defaults -> ${JSON.stringify(provs?.default || {})}`);
      }
    } catch (err) {
      debugLog(`probe: FAILED ${err.message}`);
    }
  })();
  ...
  liveHooks = {
    ...
    enabled: () => pluginOpts.liveTranscript !== false && resolveEngine(kv) !== "api",
    ...
    if (!detector) {
      detector = createHoldDetector({
        ...
        onHoldEnd() {
          ...
          doTranscribePipeline(kv, complete, client, toast, systemPrompt, autoSubmit, logger);
        },
        onTap() {
  ...
  function setVoiceMode(mode) {
    ...
    getDetector().reset();
    if (mode === "tap") {
      toast(`Voice mode: tap - press ${talkKey} to start, press again to send`);
    } else {
      toast(`Voice mode: hold - hold ${talkKey} to talk, live text appears as you speak`);
    }
  }
  ...
  function showVoiceModeDialog() {
    api.ui.dialog.replace(() =>
      api.ui.DialogSelect({
        ...
        current: getVoiceMode(),
        options: [
          {
            title: "Hold (push-to-talk)",
            value: "hold",
            onSelect() {
              setVoiceMode("hold");
              api.ui.dialog.clear();
          ...
          {
            title: "Tap (press to start, press to send)",
            value: "tap",
            onSelect() {
              setVoiceMode("tap");
              api.ui.dialog.clear();
  ...
  return [
    ...
    {
      title: "Voice: setup & input mode",
      value: "voice.mode",
      description: `First-run setup; hold/tap input mode (${talkKey})`,
      slash: { name: "voice" },
      onSelect() {
        ...
        if (kv.get("voice.setupDone")) {
          api.ui.dialog.replace(() =>
            api.ui.DialogSelect({
              ...
              options: [
                {
                  title: `Input mode (currently ${getVoiceMode()})`,
                  value: "mode",
                  onSelect() {
                    showVoiceModeDialog();
                  },
    ...
    {
      title: "STT: select engine",
      value: "stt.engine",
      description: "Choose transcription engine: nemo-speech or whisper.cpp",
      slash: { name: "stt-engine" },
      onSelect() {
        const current = resolveEngine(kv);
        const whisperOk = hasBinary("whisper-cli");
        const nemoOk = hasBinary("nemo-speech");
        const engines = [
          {
            title: `nemo-speech (Parakeet/Nemotron - better accuracy, immune to silence hallucination)${nemoOk ? "" : " - nemo-speech not found"}`,
            value: "nemo",
            onSelect() {
              kv.set("stt.engine", "nemo");
              toast("STT engine: nemo-speech (first transcription downloads the model)");
              api.ui.dialog.clear();
            },
          },
          {
            title: `whisper.cpp${whisperOk ? "" : " - whisper-cli not found"}`,
            value: "whisper",
            onSelect() {
              kv.set("stt.engine", "whisper");
              toast("STT engine: whisper.cpp");
              api.ui.dialog.clear();
            },
          },
        ];
        if (sttApiEndpoint) {
          engines.push({
            title: `Transcription API (${sttApiEndpoint})`,
            value: "api",
            onSelect() {
              kv.set("stt.engine", "api");
              toast("STT engine: transcription API");
              api.ui.dialog.clear();
            },
          });
        }
    ...
    },
    {
      ...
      value: "stt.model",
      description: "Choose transcription model for the active engine",
      slash: { name: "stt-model" },
      onSelect() {
        const engine = resolveEngine(kv);
        if (engine === "nemo") {
          const current = getNemoModel(kv);
          api.ui.dialog.replace(() =>
            api.ui.DialogSelect({
              title: "Select nemo-speech model",
              current,
              options: Object.entries(NEMO_MODELS).map(([key, v]) => ({
                ...
                onSelect() {
                  ...
                  toast(`nemo-speech model: ${v.label}`);
                  api.ui.dialog.clear();
                },
              })),
            }),
          ...
          const current = getWhisperModelName(kv);
          api.ui.dialog.replace(() =>
            api.ui.DialogSelect({
              title: "Select whisper model",
              current,
              options: Object.entries(WHISPER_MODELS).map(([key, v]) => ({
                title: v.label,
                value: key,
                onSelect() {

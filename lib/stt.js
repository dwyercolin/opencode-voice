...
export function registerSTT(api, kv, complete, opts, logger) {
  ...
  liveHooks = {
    ...
    // Opt out with liveTranscriptTarget: "toast"; liveTarget() also falls
    // back to toast on its own when clearPrompt is missing or throws.
    target: opts?.liveTranscriptTarget === "toast" ? "toast" : "prompt",
  ...
  function showVoiceMenu() {
    ...
    const options = [
      ...
      {
        title: "Close",
        description: "escape does the same",
        value: "close",
        onSelect() {
          api.ui.dialog.clear();
        },

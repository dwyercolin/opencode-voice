...
async function normalizeTranscription(complete, rawText, sessionTitle, systemPrompt, logger) {
  ...
  const result = await complete({
    system,
    prompt: `Clean up this speech-to-text transcription. Preserve its original language; do not translate it.\n\n${rawText}`,
  });
  return result;
}
...
export function registerSTT(api, kv, complete, opts, logger) {
  ...
  function installSelectedSttRuntime({ next, back, waitForInstall = false } = {}) {
    const config = getSttConfig(kv);
    const runtime = managedSttRuntime(config.backend);
    if (!runtime) {
      toast(`${config.displayName} cannot be installed automatically`, "warning");
      back?.();
      return;
    }
    api.ui.dialog.clear();
    ...
    const job = setup.installRemoteModel({
      ...
      onDone(ok) {
        ...
        remoteInstallIntent = null;
        toast(`${config.displayName} loaded`, "success");
  ...
  function showModelMissing() {
    ...
    if (readiness.status === "unavailable") {
      toast(readiness.message, "warning", 10000);
      return;
    }
    if (config.backend !== STT_BACKENDS.NEMO) {
      ...
      installSelectedSttRuntime({ back: showVoiceMenu });
      return;
    }
  ...
  const setup = createVoiceSetup({
    ...
    getVoiceMode,
    setVoiceMode,
    installRemoteRuntime: ({ next, back } = {}) =>
      installSelectedSttRuntime({ next, back, waitForInstall: true }),
  });
  ...
  // Local models and managed runtimes report their installation state in the
  // picker, so selecting a model never needs a server URL or credential.
  function currentManagedRuntimeReady(config) {
    return (
      modelSupportsSttLanguage(config, config.language) &&
  ...
  function modelRowState(key) {
    ...
    const currentConfig = getSttConfig(kv);
    ...
    if (managedSttModelCached(option.backend, option.model)) return "cached";
    if (managedSttRuntime(option.backend)) return "model-missing";
  ...
  function modelSummary() {
    ...
    if (status === "cached") return `${name} · downloaded · load to use`;
    if (status === "unsupported-language") {
      return `${name} · no ${sttLanguageLabel(config.language)} support`;
    }
    if (status === "unavailable") return `${name} · choose another model`;
    return `${name} · not ready`;
  }

  function showLanguageDialog({ next, back } = {}) {
    const current = getSttConfig(kv).language;
    const options = STT_LANGUAGE_OPTIONS.map((language) => ({
      title: language.label,
      description:
        language.id === STT_LANGUAGE_AUTO
          ? "let the selected model detect the language"
          : "prefer models with this language coverage",
      footer: language.id === current ? "selected" : language.id,
      value: language.id,
      onSelect() {
        kv.set("stt.language", language.id);
        const config = getSttConfig(kv);
        const model = sttModelOption(config.id);
        if (model && !modelSupportsSttLanguage(model, language.id)) {
          toast(
            `${config.displayName} does not list ${language.label} support - choose another model`,
            "warning",
          );
        } else {
          toast(`Transcription language: ${language.label}`);
        }
        if (next) {
          next();
          return;
    ...
    addBackRow(options, back, "keep the current language");
    menu({
  ...
  function showModelDialog({ next, back } = {}) {
    const language = getSttConfig(kv).language;
    const options = Object.entries(STT_MODELS)
      .filter(([, v]) => modelSupportsSttLanguage(v, language))
      .map(([key, v]) => {
        ...
        return {
                ...
                : status === "installing"
                  ...
                  : status === "partial"
                    ...
                    : status === "unsupported-language"
                      ? `does not support ${sttLanguageLabel(language)}`
                      : status === "cached"
                        ? "downloaded · enter loads"
          ...
          onSelect() {
            ...
            if (v.backend === STT_BACKENDS.NEMO) {
              ...
              const managed = managedSttRuntime(v.backend);
              if (managed && !managedSttRuntimeInstalled(v.backend, v.model)) {
                installSelectedSttRuntime({
                  next,
  ...
  function showTalkKeyTextDialog() {
    ...
    api.ui.dialog.replace(
      ...
      () => {
        // DialogPrompt clears itself after onConfirm. Reopening the hub after
        // that clear finishes prevents it from being closed immediately.
        queueMicrotask(showVoiceMenu);
      },
    );
  ...
  function showVoiceMenu() {
    ...
    const options = [
      ...
      },
      {
        title: "Transcription language",
        category: "Transcription",
        description: sttLanguageLabel(getSttConfig(kv).language),
        footer: getSttConfig(kv).language,
        value: "language",
        onSelect() {
          showLanguageDialog({ next: showVoiceMenu, back: showVoiceMenu });
        },
      },
      {
        title: "Microphone",

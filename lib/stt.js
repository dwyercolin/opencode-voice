...
export function registerSTT(api, kv, complete, opts, logger) {
  ...
  function handleTalkPress() {
    if (processing) {
      // "Busy" alone hid a real event on fresh installs: the first run
      // downloads the ~700 MB model inside the transcription call, and the
      // key stays locked for its duration. Say which world the user is in.
      toast(
        hasVerifiedNemoModel()
          ? "Transcribing - one moment"
          : "First run: the transcription model (~700 MB) is downloading - the talk key works once it lands",
      );
      return;
    }
    if (getVoiceMode() === "tap") {

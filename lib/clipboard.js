...
function runTool(bin, args, text) {
  return new Promise((resolve) => {
    ...
    // Registered before the write: a fast-exiting tool must not be able to
    // fire 'close' before the listener exists, which would hang the copy.
...
export async function copyToClipboard(api, text, deps = {}) {
  // Forget cached misses: installing xclip is the expected response to the
  // failure toast, and it must work on the very next copy, not on restart.
